...
class RancherClient {
    constructor(config) {
        ...
        this._request = (method, url, options) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                this.request(url, Object.assign({ method: method, json: true }, options), (err, res, json) => {
                    if (err) {
                        return reject(err);
                    }
    ...
    createContainer(container) {
        return this._request('post', '/container', { body: JSON.stringify(container) });
    };
    ...
    updateContainer(container) {
        return this._request('post', `/container/${container.id}`, { body: JSON.stringify(container) });
    }

    stopContainer(containerId, stopParams) {
        Joi.assert(containerId, Joi.string().required(), 'Must specify container id');
        return this._request('post', `/container/${containerId}/?action=stop`, { body: JSON.stringify(stopParams) });
    }
    ...
    createStack(stack) {
        return this._request('post', '/stacks', { body: JSON.stringify(stack) });
    }
    ...
    restartService(serviceId, restartParams) {
        Joi.assert(serviceId, Joi.string().required(), 'Must specify service id');
        return this._request('post', `/services/${serviceId}/?action=restart`, { body: JSON.stringify(restartParams) });
    }
    ...
    createVolume(volume) {
        return this._request('post', '/volume', { body: JSON.stringify(volume) });
    }
    getVolume(volumeId) {
