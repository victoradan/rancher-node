...
class RancherClient {
  ...
  getContainerLogs(containerId, params) {
    Joi.assert(containerId, Joi.string().required(), 'Must specify container id')
    return this._request('POST', `/container/${containerId}/?action=logs`, {json: params})
  }

  getContainerLogsPayload(hostAccess) {
    const options = {headers: {"Authorization": "Bearer " + hostAccess.token}}
    var ws = new WebSocket(hostAccess.url + '?token=' + hostAccess.token, options)

    function get(resolve, reject) {
      const result = []
      ws.on('message', (msg) => result.push(msg))
      ws.on('close', (arg) => resolve(result))
