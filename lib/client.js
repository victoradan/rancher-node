...
class RancherClient {
    constructor(config) {
        ...
        this._request = (method, url, options) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                this.request(url, Object.assign({ method: method }, options), (err, res, json) => {
                    if (err) {
                        return reject(err);
                    }
    ...
    createContainer(container) {
        return this._request('POST', '/container', { json: container });
    };
    ...
    updateContainer(container) {
        return this._request('POST', `/container/${container.id}`, { json: container });
    }

    stopContainer(containerId, stopParams) {
        Joi.assert(containerId, Joi.string().required(), 'Must specify container id');
        return this._request('POST', `/container/${containerId}/?action=stop`, { json: stopParams });
    }
    ...
    createStack(stack) {
        return this._request('POST', '/stacks', { json: stack });
    }
    ...
    restartService(serviceId, restartParams) {
        Joi.assert(serviceId, Joi.string().required(), 'Must specify service id');
        return this._request('POST', `/services/${serviceId}/?action=restart`, { json: restartParams });
    }
    ...
    createVolume(volume) {
        return this._request('POST', '/volume', { json: volume });
    }
    getVolume(volumeId) {
